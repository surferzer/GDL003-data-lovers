...
const showName = ()=>{
  ...
  let slideB=document.querySelector('.before');
  let slideN=document.querySelector('.next');

  let button = document.querySelector('.names');
  
  document.querySelector('#showData').innerHTML=window.dataLovers.filter(RICKANDMORTY.results, button,0,slideB, slideN);
...
document.querySelector('.before').addEventListener('click',showName);
document.querySelector('.next').addEventListener('click',showName);
